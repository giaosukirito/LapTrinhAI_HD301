...
public class GA_NQueenAlgo {
    ...
    // Enter your code here
        ...
        int k = 0;
        while (k++ < 2000) {
            List<Node> newReproduce = new ArrayList<> ();
            for(int i = 0; i < POP_SIZE; i++) {
                ...
                newReproduce.add (child);
            }
            population = newReproduce;
        }
    ...
    public Node getParentByTournamentSelection(int k) {
        List<Node> listRandom = new ArrayList<> ();
        for(int i = 0; i < k; i++) {
            listRandom.add (population.get (rd.nextInt (POP_SIZE)));
        }
        int h = listRandom.get (0).getH ();
        Node result = new Node ();
        for(Node node : listRandom) {
            if (node.getH () < h) {
                h = node.getH ();
                result = node;
            }
